function LB3_prepro_eegfmri_1(datafolder, filename, newSamplingRate, filterCutOff)
        ...
        %news sampling rate of 500 is clean
        ...
        EEG=pop_chanedit(EEG,'standard_1005.elc');
        ...
        for i = 1:length(EEG.event)
            if strcmp(EEG.event(i).type(1:4), 'R128')
                EEG.event(i).type = 'R128';
            end
        end
        ...
        [EEG, ~] = pop_fmrib_fastr(EEG, 70, 4, 51, 'R128', 0, 0, 0, [], [], [], 32, 'auto');
        %% downsample
        ...
        %% save HR as dataset
        EEG_ecg = pop_select(EEG, 'channel', 32);
        pop_saveset(EEG_ecg, 'filename', [basename '_ECG.set'], 'filepath', datafolder);
   ...
   %% run ICA
        ...
        EEG = eeg_checkset(EEG);
        pop_saveset(EEG, 'filename',[basename '_afterICA2.set'], ...
                         'filepath',datafolder);
